...
- The `per_sample_minmax` short-horizon no-detach sweep completed. Its strongest result is the 51-point, 1000-wavelength task: full-test `autoregressive_r2=0.4873` and `autoregressive_final_r2=0.3301`. This is the current best usable short-horizon RNN baseline.
- The controlled no-detach horizon sweep completed. With the 51-point result as reference (`autoregressive_r2=0.4873`), full-test autoregressive R2 declined to `0.4339` for 101 points and `0.3716` for 201 points. This isolates rollout length as a material limitation even within the first 10 cm.
- The read-only original-SC versus Luna data-dynamics comparison completed under `/mnt/Luna.jl-master/rnn_visual_diagnostics/data_dynamics_comparison`. Under the common original dBm mapping, Luna targets are substantially sparser (61.45% exactly zero versus 26.41%), restructure most strongly at the input boundary, and have larger shared-PCA nearest-history and next-step differences. The report is descriptive and does not claim a unique physical cause.
- The uniform RNN post-training visualization batch completed successfully in the cloud. All 12 eligible non-smoke runs now have comparable target, stepwise, autoregressive, final-spectrum, and per-step-R2 outputs under `/mnt/Luna.jl-master/rnn_visual_diagnostics/training_batch/`. The manifest records three legacy z1401 directories without retained complete prediction files and seven intentionally skipped smoke runs; no eligible visualization run failed.
...
- Fixed-horizon evaluator smoke passed on the existing Luna `z10cm_51_lambda251` task. After one epoch on 50 train and 10 test trajectories, all-start R2 was `0.790`, `0.778`, `0.764`, and `0.748` at 1--4 recursive steps. These are implementation checks only, not benchmark results.
...
- Completed raw4 Luna validation: of 20 deduplicated SAC candidates, 11 completed and 9 became non-runnable after self-compression exceeded the PPT table limit. The successful candidates had UV fraction essentially equal to one, but Luna `stats/energy` transmission was only `2.17e-20--3.02e-13` and maximum electron density was `3.33e23--1.93e24 m^-3`. Therefore UV fraction alone is a degenerate objective: it rewards near-total spectral/energy depletion into the short-wave range, not usable UV output.
- The raw4 forward model nevertheless has useful validation-set ranking for absolute spectral proxies: UV-power-proxy Spearman `0.9438` and total-power-proxy Spearman `0.9764`. The next RL reward should therefore use an absolute UV-power term plus output-power and feasibility constraints, rather than UV fraction alone. Do not start that retraining while the controlled Complex RNN data generator is occupying the cloud CPU.
- A separate fixed-distance inverse-design subtask is now running. `processed_t650_global_log_raw4_z5cm` extracts the common temporal slice at `z=5 cm` (index 50 of 501 saved planes), refits global-log target statistics from its train split, and trains `raw4_z5cm_banded_mlp_v1`. It does not replace the 50 cm task; it tests whether useful UV can be optimized at its formation distance before later depletion dominates.
