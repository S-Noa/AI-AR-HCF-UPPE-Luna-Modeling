...
- A uniform RNN post-training visualization batch is prepared for every completed non-smoke result that retains both prediction `.mat` files. It writes comparable target, stepwise, autoregressive, final-spectrum, and per-step-R2 outputs under `/mnt/Luna.jl-master/rnn_visual_diagnostics/training_batch/` and records skipped or failed runs in `batch_manifest.csv`.
