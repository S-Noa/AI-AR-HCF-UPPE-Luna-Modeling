...
- A uniform RNN post-training visualization batch is running in the cloud for every completed non-smoke result that retains both prediction `.mat` files. It discovered 12 eligible runs, three legacy z1401 directories without retained complete prediction files, and seven smoke runs that are intentionally skipped. It writes comparable target, stepwise, autoregressive, final-spectrum, and per-step-R2 outputs under `/mnt/Luna.jl-master/rnn_visual_diagnostics/training_batch/` and records skipped or failed runs in `batch_manifest.csv`.
