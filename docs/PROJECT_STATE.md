...
- The next controlled horizon sweep runs the same recipe on `z10cm_101` and `z10cm_201` with 1000 wavelengths. It isolates the effect of increasing rollout length while holding target normalization, conditioning, and training strategy fixed.
- A read-only original-SC versus Luna data-dynamics comparison is queued behind the horizon sweep. It uses the common original dBm target mapping and will generate a PPT overview, representative ground-truth maps, spectral occupancy, z-dynamics, local-window ambiguity, and a cautious quantitative report under `/mnt/Luna.jl-master/rnn_visual_diagnostics/data_dynamics_comparison`.
