% Plot higher-order soliton spectral evolution matched with experiments

clear all; close all; clc

load('../results/full_HOS_expt_spec_lin_126_120e.mat')
%load('../results/full_test_results.mat')

s = size(Y_submit,2);

isteps = steps - window_size;
...
for ii = 1:length(num_vec)
    ...
    correct_evo_p = 10*log10(correct_evo_p/max(max(correct_evo_p)));
    predicted_evo_p = 10*log10(predicted_evo_p/max(max(predicted_evo_p)));
    correct_evo_p(correct_evo_p < -40) = -40;
    predicted_evo_p(predicted_evo_p < -40) = -40;
    
    subplot(3,3,1),plot(10*log10(correct_evo(:,end)/max(correct_evo(:,end))))
    xlabel(xl),ylabel(yl)
    xlim([1 s]), ylim(ylimit)
    title(['correct evo: ' num2str(ii)])
    
    subplot(3,3,2),plot(10*log10(predicted_evo(:,end)/max(predicted_evo(:,end))))
    xlabel(xl),ylabel(yl)
    xlim([1 s]), ylim(ylimit)
    title('predicted')
    
    subplot(3,3,3),plot(correct_evo(:,end)-predicted_evo(:,end))
    error = 10*log10(predicted_evo(:,end)/max(predicted_evo(:,end)))-10*log10(correct_evo(:,end)/max(correct_evo(:,end)));
    subplot(3,3,3),plot(error)
    xlabel(xl),ylabel(el)
